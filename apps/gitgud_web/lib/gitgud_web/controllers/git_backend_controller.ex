defmodule GitGud.Web.GitBackendController do
  ...
  def info_refs(conn, %{"user" => username, "repo" => path, "service" => service}) do
    if repo = RepositoryQuerySet.user_repository(username, path),
  ...
  defp git_info_refs(conn, repo, service) do
    case System.cmd(service, ["--advertise-refs", Repository.git_dir(repo)]) do
      {resp, 0} ->
        conn
  ...
  defp git_pack(conn, repo, service) do
    ...
    |> send_resp(:ok, execute_port(service, Repository.git_dir(repo), body))
  end
