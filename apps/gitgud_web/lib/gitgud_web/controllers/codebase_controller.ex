defmodule GitGud.Web.CodebaseController do
  ...
  def branches(conn, %{"username" => username, "repo_name" => repo_name} = _params) do
    if repo = RepoQuery.user_repo(username, repo_name, viewer: current_user(conn)) do
      with {:ok, head} <- Repo.git_head(repo),
           ...
           do: render(conn, "branch_list.html", repo: repo, head: head, branches: branches)
    end || {:error, :not_found}
  end
  ...
  def tags(conn, %{"username" => username, "repo_name" => repo_name} = _params) do
    if repo = RepoQuery.user_repo(username, repo_name, viewer: current_user(conn)) do
      with {:ok, tags} <- Repo.git_tags(repo),
           do: render(conn, "tag_list.html", repo: repo, tags: tags)
    end || {:error, :not_found}
  end
  ...
  def commit(conn, %{"username" => username, "repo_name" => repo_name, "oid" => oid} = _params) do
    if repo = RepoQuery.user_repo(username, repo_name, viewer: current_user(conn)) do
      with {:ok, commit} <- Repo.git_object(repo, oid),
           do: render(conn, "commit.html", repo: repo, commit: commit)
    end || {:error, :not_found}
  end
  ...
  def history(
        conn,
        %{"username" => username, "repo_name" => repo_name, "revision" => revision} = _params
      ) do
    if repo = RepoQuery.user_repo(username, repo_name, viewer: current_user(conn)) do
      with {:ok, object, reference} <- Repo.git_revision(repo, revision),
           {:ok, history} <- Repo.git_history(object),
           do:
  ...
  def history(conn, %{"username" => username, "repo_name" => repo_name} = _params) do
    if repo = RepoQuery.user_repo(username, repo_name, viewer: current_user(conn)) do
      with {:ok, reference} <- Repo.git_head(repo),
           {:ok, history} <- Repo.git_history(reference),
           do: render(conn, "commit_list.html", repo: repo, revision: reference, commits: history)
    end || {:error, :not_found}
  end
  ...
  def tree(
        conn,
        %{"username" => username, "repo_name" => repo_name, "revision" => revision, "path" => []} =
          _params
      ) do
    if repo = RepoQuery.user_repo(username, repo_name, viewer: current_user(conn)) do
      with {:ok, object, reference} <- Repo.git_revision(repo, revision),
  ...
  def tree(
    ...
    if repo = RepoQuery.user_repo(username, repo_name, viewer: current_user(conn)) do
      with {:ok, object, reference} <- Repo.git_revision(repo, revision),
           ...
           {:ok, tree} <- GitTreeEntry.target(tree_entry),
           do:
  ...
  def blob(
    ...
    if repo = RepoQuery.user_repo(username, repo_name, viewer: current_user(conn)) do
      with {:ok, object, reference} <- Repo.git_revision(repo, revision),
           ...
           {:ok, blob} <- GitTreeEntry.target(tree_entry),
           do:
  ...
  defp stats!(%{repo: repo} = revision) do
    with {:ok, history} <- Repo.git_history(revision),
         {:ok, branches} <- Repo.git_branches(repo),
         {:ok, tags} <- Repo.git_tags(repo),
         do: %{
           commits: Enum.count(history.enum),
           branches: Enum.count(branches.enum),
           tags: Enum.count(tags.enum)
         }
  end
end
