defmodule GitGud.Web.SessionControllerTest do
  ...
  describe "when user exists" do
    ...
    test "creates session with valid credentials", %{conn: conn, user: user} do
      conn = post(conn, session_path(conn, :create), session: %{email_or_username: user.email, password: "qwertz"})
      assert get_flash(conn, :info) == "Logged in."
      assert redirected_to(conn) == user_path(conn, :show, user)
      conn = post(conn, session_path(conn, :create), session: %{email_or_username: user.username, password: "qwertz"})
      assert get_flash(conn, :info) == "Logged in."
      assert redirected_to(conn) == user_path(conn, :show, user)
    end

    test "fails to create session with invalid credentials", %{conn: conn, user: user} do
      conn = post(conn, session_path(conn, :create), session: %{email_or_username: user.email, password: "qwerty"})
      assert get_flash(conn, :error) == "Wrong login credentials"
      assert html_response(conn, 401) =~ ~s(<h1 class="title">Login</h1>)
      conn = post(conn, session_path(conn, :create), session: %{email_or_username: user.username, password: "qwerty"})
      assert get_flash(conn, :error) == "Wrong login credentials"
      assert html_response(conn, 401) =~ ~s(<h1 class="title">Login</h1>)
    end
    ...
    test "deletes session", %{conn: conn, user: user} do
      ...
      assert redirected_to(conn) == landing_page_path(conn, :index)
    end
  end
