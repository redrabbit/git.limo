defmodule GitGud.RepoQueryTest do
  ...
  test "gets single user repository", %{repos: repos} do
    for repo <- repos do
      ...
      assert repo.id == RepoQuery.user_repo(repo.owner.login, repo.name, preload: :maintainers).id
  ...
  test "gets multiple repositories from single user", %{repos: repos} do
    for {user, repos} <- Enum.group_by(repos, &(&1.owner)) do
      ...
      assert Enum.all?(RepoQuery.user_repos(user.login, preload: :maintainers), fn repo -> repo.id in Enum.map(repos, &(&1.id)) end)
  ...
  test "gets multiple repositories from multiple users", %{repos: repos} do
    ...
    assert Enum.all?(RepoQuery.user_repos(Enum.map(users, &(&1.login)), preload: :maintainers), fn repo -> repo.id in Enum.map(repos, &(&1.id)) end)
