defmodule GitGud.Repo do
  ...
  schema "repositories" do
    ...
    many_to_many :maintainers, User,
      join_through: "repositories_maintainers",
      on_replace: :delete,
  ...
  """
  @spec create(map | keyword, keyword) ::
          {:ok, t, Git.repo()} | {:error, Ecto.Changeset.t() | term}
    ...
    case update(repo, params) do
      ...
      {:error, reason} ->
        raise File.Error,
          ...
          path: IO.chardata_to_string(workdir(repo))
    end
  end
  ...
  def empty?(%__MODULE__{} = repo) do
    with {:ok, handle} <- Git.repository_open(workdir(repo)), do: Git.repository_empty?(handle)
  end
  ...
  def git_head(%__MODULE__{} = repo) do
    with {:ok, handle} <- Git.repository_open(workdir(repo)),
         {:ok, name, shorthand, oid} <- Git.reference_resolve(handle, "HEAD"),
         do: {:ok, resolve_reference({name, shorthand, :oid, oid}, {repo, handle})}
  end
  ...
  def git_reference(%__MODULE__{} = repo, "/refs/" <> _suffix = name) do
    with {:ok, handle} <- Git.repository_open(workdir(repo)),
         {:ok, shorthand, :oid, oid} <- Git.reference_lookup(handle, name),
         do: {:ok, resolve_reference({name, shorthand, :oid, oid}, {repo, handle})}
  end
  ...
  def git_references(%__MODULE__{} = repo, glob \\ :undefined) do
    with {:ok, handle} <- Git.repository_open(workdir(repo)),
         ...
         do: {:ok, Stream.map(stream, &resolve_reference(&1, {repo, handle}))}
  end
  ...
  def git_tag(%__MODULE__{} = repo, tag_name) do
    ...
    with {:ok, handle} <- Git.repository_open(workdir(repo)),
         {:ok, shorthand, :oid, oid} <- Git.reference_lookup(handle, name),
         do: {:ok, resolve_tag({name, shorthand, :oid, oid}, {repo, handle})}
  end
  ...
  def git_tags(%__MODULE__{} = repo) do
    with {:ok, handle} <- Git.repository_open(workdir(repo)),
         ...
         {:ok, stream} <- Git.enumerate(stream),
         do: {:ok, Stream.map(stream, &resolve_tag(&1, {repo, handle}))}
  end
  ...
  def git_object(%__MODULE__{} = repo, oid) do
    ...
    with {:ok, handle} <- Git.repository_open(workdir(repo)),
         {:ok, obj_type, obj} <- Git.object_lookup(handle, oid),
         do: {:ok, resolve_object({obj, obj_type, oid}, {repo, handle})}
  end
  ...
  def git_revision(%__MODULE__{} = repo, revision) do
    with {:ok, handle} <- Git.repository_open(workdir(repo)),
         {:ok, obj, obj_type, oid, name} <- Git.revparse_ext(handle, revision),
         do:
           {:ok, resolve_object({obj, obj_type, oid}, {repo, handle}),
            resolve_reference({name, nil, :oid, oid}, {repo, handle})}
  end
  ...
  def git_tree(%GitReference{name: name, prefix: prefix, repo: repo, __git__: handle} = _revision) do
    with {:ok, :commit, _oid, commit} <- Git.reference_peel(handle, prefix <> name, :commit),
         {:ok, oid, tree} <- Git.commit_tree(commit),
         do: {:ok, %GitTree{oid: oid, repo: repo, __git__: tree}}
  end
  ...
  def git_tree(%GitCommit{repo: repo, __git__: commit} = _revision) do
    with {:ok, oid, tree} <- Git.commit_tree(commit),
         do: {:ok, %GitTree{oid: oid, repo: repo, __git__: tree}}
  end
  ...
  def push(
        %__MODULE__{} = repo,
        %User{} = user,
        %ReceivePack{repo: handle, pack: pack, cmds: cmds} = _receive_pack
      ) do
      ...
      {:ok, odb} ->
        ...
        :ok =
          Phoenix.PubSub.broadcast(
            GitGud.Web.PubSub,
            "repos:#{repo.id}",
            {:push, %{repo_id: repo.id, user_id: user.id, cmds: cmds, oids: oids}}
          )
  ...
  defp init_maintainer(%{insert: repo}) do
    Multi.insert_all(Multi.new(), :init_maintainer, "repositories_maintainers", [
      [repo_id: repo.id, user_id: repo.owner_id]
    ])
  end
  ...
  defp resolve_history(oid, {repo, handle}) do
    with {:ok, walk} <- Git.revwalk_new(handle),
         ...
         {:ok, stream} <- Git.enumerate(stream),
         do: {:ok, Stream.map(stream, &resolve_object(&1, {repo, handle}))}
  end
  ...
  defp resolve_object({blob, :blob, oid}, {repo, _handle}),
    do: %GitBlob{oid: oid, repo: repo, __git__: blob}

  defp resolve_object({commit, :commit, oid}, {repo, _handle}),
    do: %GitCommit{oid: oid, repo: repo, __git__: commit}

  defp resolve_object({tree, :tree, oid}, {repo, _handle}),
    do: %GitTree{oid: oid, repo: repo, __git__: tree}
